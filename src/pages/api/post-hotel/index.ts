...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
  ...
  // const images = JSON.parse(req.body.files);

  // const list = await Promise.all(images.map(async (imge: { url: any; }) => {
  //                       const response = await cloudinary.uploader.upload(imge.url);
  //                       return {publicId: response.public_id, url: response.secure_url};
  //               }));

  // req.body.pImage = JSON.parse(req.body.pImage).concat(list);
